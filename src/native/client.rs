...
//!
//! Works against the native bridge at /usr/local/etc/xray/config.json (no
//! Amnezia clientsTable indirection). Each method edits the config via `jq`
//! over SSH, then reloads xray.
